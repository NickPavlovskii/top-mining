...
<script setup lang="ts">
   ...
   * @param {String} [prependIcon = ''] - URL иконки слева от текста
   * @param {String} [appendIcon = ''] - URL иконки справа от текста
   * @param {String} [variant = 'primary'] - вариант оформления: primary, secondary и др.
   * @param {String} [size = 'big'] - размер кнопки
   * @param {String} [surface = 'light'] - поверхность (светлая/тёмная)
   * @param {String} [preset = ''] - имя готового пресета стилей
   ...
   * @param {Boolean} [loading = false] - состояние загрузки со спиннером
   */
...
<style scoped lang="css">
  ...
  .top-mining-button.q-btn :deep(.q-btn__content) {
    display: inline-flex;
    align-items: center;
  ...
  .top-mining-button {    --tm-btn-orange: var(--tm-orange);
  ...
  .top-mining-button.q-btn.top-mining-button--contact-pill.top-mining-button--primary.top-mining-button--surface-dark {
    --tm-btn-bg: var(--tm-orange-accent-gradient-horizontal);
    --tm-btn-border: transparent;
    --tm-btn-color: var(--tm-white);
    width: 100%;
  ...
  .top-mining-button.q-btn.top-mining-button--big {
    min-width: 200px;
    width: max-content;
    max-width: 100%;
    min-height: 48px;
  ...
  .top-mining-button.q-btn.top-mining-button--small {
    min-height: 40px;
    padding: 0 18px;
    font-size: 11px;
  }
