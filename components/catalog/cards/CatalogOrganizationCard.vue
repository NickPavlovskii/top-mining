<template>
  <article :class="cardClass">
      ...
      <div class="catalog-org-card__top top-card-block">
        <div
          :class="[
            'catalog-org-card__logo image-card-wrap',
            {
              ...
              'catalog-org-card__logo--dark': isGridCard && !isLightLogo,
            },
          ]"
        >
          <img
        ...
        <div class="catalog-org-card__header">
          <div class="catalog-org-card__title-wrap title-card-wrap">
            ...
            <p class="catalog-org-card__name title-card">
              {{ displayName }}
            </p>
          </div>
    ...
    <nuxt-link
      v-if="detailHref"
      :to="detailHref"
      class="catalog-org-card__overlay"
      :aria-label="organization.name"
    />
    <button
      v-else
      type="button"
      class="catalog-org-card__overlay catalog-org-card__overlay--button"
      :aria-label="organization.name"
      @click="openMissingDetail"
    />
  </article>
</template>
...
<script setup lang="ts">
  ...
  const props = withDefaults(
    defineProps<{
      ...
      /** Раскладка: carousel или grid, по умолчанию carousel */
  ...
  /** Событие logoLoad — загрузка логотипа организации. */
  ...
  const showLogoImage = computed(
    () => Boolean(props.organization.logoUrl) && !logoFailed.value,
  )
  ...
  const showRatingRow = computed(() => {
    const { hasPublicRating, rating, reviewCount } = props.organization
    return Boolean(hasPublicRating) && rating > 0 && reviewCount > 0
  })
  ...
  function isWideTag(tag: string) {
    return (
      tag.startsWith('Добываемые монеты:')
      || tag.startsWith('Поддержка монет:')
      || tag.length > 40
    )
  }
...
<style scoped>
  ...
  .catalog-org-card__overlay--button {
    width: 100%;
    padding: 0;
    border: 0;
  ...
  .catalog-org-card--rich .catalog-org-card__overlay {
    border-radius: 30px;
  }
  ...
  .catalog-org-card--rich .catalog-org-card__logo--dark {
  ...
  .catalog-org-card--rich .catalog-org-card__logo img {
    border-radius: 16px;
  }
  ...
  .catalog-org-card__logo-fallback-icon {
    width: 36%;
    height: 36%;
    min-width: 22px;
    min-height: 22px;
  ...
  .catalog-org-card__logo--light img {
    object-fit: contain;
    padding: 8px;
    box-sizing: border-box;
  }
  ...
  .catalog-org-card--rich .catalog-org-card__title-wrap {
    width: 100%;
    min-width: 0;
    overflow: hidden;
  }

  .catalog-org-card--rich .catalog-org-card__header {
    flex: 1;
    min-width: 0;
    overflow: hidden;
  }
  ...
  .catalog-org-card:has(.catalog-org-card__overlay):hover .catalog-org-card__name {
    color: var(--tm-orange);
  }
  ...
  .catalog-org-card__feature > span:last-child {
    min-width: 0;
    overflow-wrap: anywhere;
  }
